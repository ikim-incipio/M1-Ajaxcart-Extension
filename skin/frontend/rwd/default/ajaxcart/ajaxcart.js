jQuery(document).on('click', '.button.btn-cart', function(e){
	e.preventDefault();

	var qty = jQuery(this).parent().siblings('.qty-wrapper').children('.qty').val();
	if (qty == parseInt(qty, 10) && parseInt(qty) > 0){
		var data = 'isAjax=1';
		var currentUrl = window.location.href;
		var baseUrl = currentUrl.substring(0, currentUrl.substring(7, currentUrl.length).indexOf('/')+8);
		var url = baseUrl+'ajaxcart/simple';
		...
		jQuery.ajax({
			...
			jQuery('#ajax_alert .ajax-alert-msg').text('Unknown error has occurred: '+statusText);
			jQuery('#ajax_alert').show();
			setTimeout(function(){
				jQuery('#ajax_alert').hide();
			}, 2000);
		...
		jQuery('#ajax_alert .ajax-alert-msg').text('Quantity must be integer and greater than 0.');
		jQuery('#ajax_alert').show();
		setTimeout(function(){
			jQuery('#ajax_alert').hide();
		}, 2000);
...
jQuery(document).on('click', '.button.btn-popup.config', function(e){
	...
	if (jQuery('.products-grid')[0]){
		...
		var html = jQuery(this).parent().parent().parent().parent().parent().html();
	}
...
jQuery(document).on('click', '.button.btn-ajaxcart.config', function(e){
	...
	if (qty == parseInt(qty, 10) && parseInt(qty) > 0){
		...
		if (jQuery('.products-grid')[0]){
			...
			var selected = jQuery(this).parent().parent().siblings('.product-primary').children('.configurable-swatch-list-ajax').children('.selected');
		}
