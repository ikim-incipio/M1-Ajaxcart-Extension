...
var ConfigurableSwatchesListAjax = {
    swatchesByProduct: {},

    init: function()
    {
        var that = this;
        $j('.configurable-swatch-list-ajax li').each(function() {
            that.initSwatch(this);
            var $swatch = $j(this);
            // if ($swatch.hasClass('filter-match')) {
            //     that.handleSwatchSelect($swatch);
            // }
            if ($swatch.hasClass('first-swatch-option')){
                that.handleSwatchSelect($swatch);
            }
        });
    },
    ...
    handleSwatchSelect: function($swatch)
    {
        ...
        var label;
        var isAjax = jQuery('.lightBoxWrapper')[0] != undefined ? true : false;
        if (label = $swatch.data('option-label')) {
            ConfigurableMediaImages.swapListImageByOption(productId, label, isAjax);
        }
...
$j(document).on('configurable-media-images-ajax-init', function(){
    ConfigurableSwatchesListAjax.init();
});
